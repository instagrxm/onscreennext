...
class PMAccountController extends Controller
{
    ...
    private function save()
    {
        ...
        $logged_in = false;
        try {
            $resp = $Instagram->login($this->username, $this->password);

            if ($resp !== null && $resp->isTwoFactorRequired()) {
                $this->resp->result = 2;
                $this->resp->twofa_required = true;
                $this->resp->msg = __(
                    "Enter the code sent to your number ending in %s", 
                    $resp->getTwoFactorInfo()->getObfuscatedPhoneNumber());
        ...
        $ig_pic_url = $resp->getLoggedInUser()->getProfilePicUrl();      
        $url_parts = parse_url($ig_pic_url);   
        $ext = strtolower(pathinfo($url_parts['path'], PATHINFO_EXTENSION));
        $filename = "profile-pic-".$resp->getLoggedInUser()->getUsername().".".$ext;
    ...
    protected function twofa()
    {
        ...
        // Update proxy use count
        if ($proxy && $this->system_proxy == true) {
    ...
    protected function challenge()
    {
		...
		try {
            $resp = $Instagram->finishChallengeLogin($username, $password, $api_path, $security_code);
        } catch (InstagramAPI\Exception\CheckpointRequiredException $e) {
